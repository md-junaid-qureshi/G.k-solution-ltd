  ...
  return (
    <div className="flex flex-col bg-[var(--brand-white)] min-h-screen">
      ...
      <section className="py-20 bg-[var(--brand-light)] border-b border-[var(--brand-border)]">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center pb-16 border-b border-[var(--brand-border)]">
            <div className="lg:col-span-6 space-y-4">
              <span className="text-xs font-semibold tracking-[0.2em] text-[var(--brand-brass)] uppercase">
                The Firm
              </span>
              <h2 className="font-heading text-3xl sm:text-4xl text-[var(--brand-dark)] tracking-tight">
            ...
            <div className="lg:col-span-6 relative aspect-[16/11] overflow-hidden border border-[var(--brand-border)] shadow-xl bg-[var(--brand-dark)]">
              <Image
              ...
              <div className="absolute bottom-4 left-4 right-4 p-3 bg-[var(--brand-dark)]/90 backdrop-blur-sm border border-[var(--brand-border)]/40 text-xs text-[var(--brand-light)] font-mono">
          ...
          <div className="mt-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {companyStats.map((stat) => (
              <div
                key={stat.label}
                className="p-6 bg-[var(--brand-white)] border border-[var(--brand-border)] transition-all hover:border-[var(--brand-brass)] hover:shadow-md"
              >
                <span className="font-heading text-4xl sm:text-5xl font-light text-[var(--brand-dark)]">
                  {stat.value}
                </span>
                <span className="mt-2 block text-xs font-semibold uppercase tracking-[0.15em] text-[var(--brand-brass)] font-mono">
                  {stat.label}
                </span>
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          ...
          <div className="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {corePhilosophy.map((item, idx) => (
              ...
              >
                <span className="font-mono text-xs font-semibold text-[var(--brand-brass)]">
                  0{idx + 1}. PILLAR
                </span>
                <h3 className="mt-3 font-heading text-xl text-[var(--brand-dark)]">
                  {item.title}
                </h3>
      ...
      {/* ── 4. THE 5-STEP OPERATIONAL PROCESS ─────────────────── */}
      <section className="py-24 bg-[var(--brand-dark)] text-[var(--brand-light)] border-b border-[var(--brand-border)]">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="max-w-2xl">
            <span className="text-xs font-semibold tracking-[0.2em] text-[var(--brand-brass)] uppercase">
              Operational Governance
            </span>
            <h2 className="mt-2 font-heading text-3xl sm:text-4xl lg:text-5xl text-[var(--brand-light)] tracking-tight">
          ...
          <div className="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {fiveStepProcess.map((step) => (
              <div
                key={step.step}
                className="p-6 border border-[var(--brand-border)]/30 bg-[var(--brand-light)]/5 flex flex-col justify-between"
              >
                <div>
                  <span className="font-heading text-3xl text-[var(--brand-brass)] font-light">
                    {step.step}
                  </span>
                  <h3 className="mt-3 font-mono text-sm font-semibold uppercase tracking-wider text-[var(--brand-light)]">
                    {step.name}
                  </h3>
                  <p className="mt-2 text-xs text-[var(--brand-muted)] leading-relaxed">
                    {step.desc}
                ...
                <div className="mt-6 pt-3 border-t border-[var(--brand-border)]/20 text-[10px] text-[var(--brand-brass)] font-mono">
                  Stage Sign-off
                </div>
              </div>
            ))}
      ...
      <section className="py-24 bg-[var(--brand-white)]">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="max-w-2xl">
          ...
          <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-8">
            {leadershipTeam.map((leader) => (
              <div
                key={leader.name}
                className="group flex flex-col bg-[var(--brand-light)] border border-[var(--brand-border)] overflow-hidden transition-all duration-300 hover:border-[var(--brand-brass)] hover:shadow-xl"
              >
                {/* Photo Frame */}
                <div className="relative aspect-[4/3] w-full bg-[var(--brand-dark)] overflow-hidden">
                  <Image
                    src={leader.image}
                    alt={leader.name}
                    fill
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-[var(--brand-dark)]/70 via-transparent to-transparent" />
                  <div className="absolute top-3 right-3 bg-[var(--brand-dark)]/90 backdrop-blur-sm text-[var(--brand-brass)] text-[10px] font-mono uppercase px-2 py-0.5 border border-[var(--brand-border)]/40">
                    {leader.experience}
                  </div>
                </div>
                ...
                <div className="p-6 flex flex-1 flex-col justify-between">
                  <div>
                    <h3 className="font-heading text-xl text-[var(--brand-dark)] group-hover:text-[var(--brand-brass)] transition-colors">
                      {leader.name}
                    </h3>
                    <p className="text-xs font-semibold uppercase tracking-wider text-[var(--brand-brass)] font-mono mt-1">
                      {leader.role}
                    </p>
                    <p className="mt-3 text-xs text-[var(--brand-muted)] leading-relaxed font-sans">
                      {leader.focus}
                    </p>
                  </div>

                  <div className="mt-6 pt-4 border-t border-[var(--brand-border)] text-[11px] font-mono text-[var(--brand-muted)]">
      ...
      {/* ── 6. BOTTOM CTA STRIP ───────────────────────────────── */}
      <section className="py-20 bg-[var(--brand-dark)] text-[var(--brand-light)] border-t border-[var(--brand-border)]">
        <div className="mx-auto max-w-7xl px-6 lg:px-8 text-center flex flex-col items-center">
          <span className="text-xs font-semibold tracking-[0.25em] text-[var(--brand-brass)] uppercase">
            Experience Our Work
          </span>
          <h2 className="mt-3 font-heading text-3xl sm:text-4xl lg:text-5xl text-[var(--brand-light)] tracking-tight max-w-3xl">
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-center gap-4">
            <Link
              href="/portfolio"
              className="inline-flex items-center justify-center gap-2 bg-[var(--brand-brass)] px-8 py-3.5 text-xs font-semibold uppercase tracking-[0.2em] text-[var(--brand-dark)] hover:bg-[var(--brand-light)] transition-colors"
            >
            ...
            <Link
              href="/contact"
              className="inline-flex items-center justify-center gap-2 border border-[var(--brand-border)]/50 px-8 py-3.5 text-xs font-semibold uppercase tracking-[0.2em] text-[var(--brand-light)] hover:border-[var(--brand-brass)] transition-colors"
            >
              Get In Touch
